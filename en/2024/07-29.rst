...
A customer expressed the wish to connect two locations with one 'portal'.
...
* The Agent is programmed in `GoLang <https://go.dev>`_ - a solution in this programming language would be easy to integrate

* The agent uses a Chromium browser for the display. The portal should also be streamed via this browser

WebRTC
======

WebRTC is used today by meeting solutions, for example.

`The Protocol <https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API/Protocols>`_ allows clients to communicate directly via a peer-to-peer connection.
